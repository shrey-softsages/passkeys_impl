...
import 'package:passkeys_impl/relying_party_server.dart';

final relyingPartServer = PersonalRelyingPartyServer();

void main() async {
  WidgetsFlutterBinding.ensureInitialized();

  relyingPartServer.init();
  runApp(const MyApp());
}
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _onclick() async {
    ...
    try {
      if (_pageMode == PageMode.registration) {
        bool isSupported = await widget._auth.isSupported();
        if (isSupported) {
          print("Supporteddddddddddd");
        } else {
          print("Not supporteddddddddddddddddd");
        }
        final response =
